...
MyOpenFile(FSSpec *myFSS) {
  if (gRegistered) {
    ...
    DoError(StandardAlert(kAlertStopAlert,
                          "\pYou cannot use custom level files using the "
                          "unregistered version of Reckless Drivin'.",
                          "\pPlease pay the registration fee for Reckless "
                          "Drivin to get rid of this message.",
                          &alertParam, &alertHit));
  }
}
...
OSErr MyGotRequiredParams(AppleEvent *theAppleEvent) {
  ...
  if (myErr == errAEDescNotFound) // you got all the required
                                  // parameters
    return noErr;
  else if (myErr == noErr) // you missed a required parameter

...
*/
EXPORT void lzrw3_compress(UWORD action, UBYTE *wrk_mem, UBYTE *src_adr,
                           uint32_t src_len, UBYTE *dst_adr,
                           uint32_t *p_dst_len) {
  switch (action) {
...
LOCAL void lzrw3_compress_compress(UBYTE *p_wrk_mem, UBYTE *p_src_first,
                                   uint32_t src_len, UBYTE *p_dst_first,
                                   uint32_t *p_dst_len)
...
LOCAL void lzrw3_compress_decompress(UBYTE *p_wrk_mem, UBYTE *p_src_first,
                                     uint32_t src_len, UBYTE *p_dst_first,
                                     uint32_t *p_dst_len)
/* Input  : Hand over the required amount of working memory in p_wrk_mem.     */
/* Input  : Specify input block using p_src_first and src_len.                */
...
void LZRWDecodeHandle(Handle *handle) {
  ...
  unsigned char *working_mem = malloc(sizeof *working_mem * lzrw3_req_mem());
  if (!working_mem) {
    return;
  }
  ...
  uint32_t dst_len;
  lzrw3_compress(COMPRESS_ACTION_DECOMPRESS, working_mem, **handle, handle_len,
                 dst_mem, &dst_len);
  free(working_mem);

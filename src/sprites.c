...
// int SlopeInit(float cx, float cy, int *y1, int *y2, float dirCos, float
// dirSin,
...
//   SlopeInitUVLine(vertices[(hi - 1) & 3], vertices[(hi - 2) & 3], dudx,
//   dvdx); SlopeInitLine(vertices[hi], vertices[(hi + 1) & 3]);
...
// void DrawSpriteRotatedClipped(tSpriteHeader *sprite, int dudx, int dvdx, int
// y,
//                               int y2) {
//   int vMask = sprite->ySize - 1 << 8;
//   int vShift = 8 - sprite->log2xSize;
...
// void DrawSpriteRotatedTranslucent16(tSpriteHeader16 *sprite, int dudx, int
// dvdx,
//                                     int y, int y2) {
//   int vMask = sprite->ySize - 1 << 8;
//   int vShift = 8 - sprite->log2xSize;
...
//       else if (SlopeInit(cx, cy, &y, &y2, dirCos, dirSin, sprite, dudx,
//       dvdx))
...
// void XDistortSprite16(tSpriteHeader16 *sprite, int startY, int endY, int
// startX,
//                       int endX, int dir, float damage) {
//   uint16_t *lineStart;
//   int line;
...
// int XDistortSprite(int id, int startY, int endY, int startX, int endX, int
// dir,
//                    float damage) {
...
//             columnStart[i * sprite->xSize] =
//             gLightningTab[shade][pixelValue];
//           }
//         }
//         for (i = 0; i <= (int)crunch; i++)
...
// void YDistortSprite16(tSpriteHeader16 *sprite, int startX, int endX, int
// startY,
//                       int endY, int dir, float damage) {
...
// int YDistortSprite(int id, int startX, int endX, int startY, int endY, int
// dir,
//                    float damage) {
//   tSpriteHeader *sprite;
//   if (id - 128 < kNumSprites) {
...
//                 int)(kLightValues *
//                      (0.6 + 0.2 * invSize * (fabs(x - xPos) + fabs(y -
//                      yPos))))]
//                                               [sprite->data[offs]];
// }
